...
while True:
   ...
   # map
      ...
      try:               # check to see if the word is already in the table
         crsr.execute("IF EXISTS (SELECT unfamilarity FROM vocaTable WHERE word = \"" + chArray[0] +"\")")
   ...
   # answer
      ...
      try:
         for i in ans:
            ...
            meaning = crsr.fetchall()
            print bcolors.WHITE + str(meaning[0][0]) + (25-len(str(meaning[0][0])))*" " + str(meaning[0][1]) + bcolors.ENDC
      except:
